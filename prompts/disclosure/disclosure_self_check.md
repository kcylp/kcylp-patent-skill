...
- [ ] **字体字号**：正文宋体小四（12pt）、标题黑体二号/三号/四号、图注宋体五号；行距 25 磅；页边距 A4 标准
- [ ] **配色**：全文黑白/灰度，无彩色文字；图片黑白线稿，无彩色标注
- [ ] **图号一一对应**：正文「如图 N」与配图内标注的数字代号逐条核对，确保完全一致；禁止正文有图号但配图无标注，或配图有标注但正文未引用
- [ ] **部件序号对应**：实用新型部件序号引出线与 `structure_schema.parts` 的 id 完全一致；跨图同一部件序号不改
- [ ] **SVG 质量**：黑白（黑线白底）、最高清晰度、放大不失真、线条 0.5–1.0pt、标注清晰可读
- [ ] **图片排版**：有序排列、无重叠、无挤压、留足间距、图注紧跟图片
