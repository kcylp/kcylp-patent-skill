...
## 7.7 专利交底书质量硬标准（全类型必遵）

以下标准在任何情况下不得妥协，是交底书能否直接递交的底线：

### 7.7.1 字体与字号

正文、标题、图注、说明书均须使用符合 CNIPA 规范的字体与字号：
...
- 图片为**黑白线稿**或灰度照片，禁止彩色标注/彩色高亮
- 图中文字如需区分层次，通过**字号/线型/灰度**实现，不用颜色

### 7.7.3 图号与配图一一对应（硬性）

- 正文中引用的**图号**（如「如图 1 所示」「参见图 3-1」）必须与配图中**实际标注的数字代号一一对应**
- **禁止**正文提"图 3"但配图里没有"3"
- **禁止**配图有"5"但正文从未提及
- 图号标注必须在配图内以**数字/罗马数字**清晰标注，不得只存在于正文
- 实用新型的部件序号引出线必须与 `structure_schema.parts` 的 id 完全一致

### 7.7.4 SVG 图片质量（硬性）

所有交底书中的线框图、结构图、流程图必须为 **SVG 格式**，且满足：

- **黑白**：黑线白底，无彩色
- **最高清晰度**：无论怎么放大都不能失真、不能模糊
- **线条粗细**：主线 0.5–1.0pt，辅线/引出线 0.3–0.5pt
- **标注清晰**：部件序号、方向箭头、尺寸标注必须可读
- **文字嵌入**：图中文字以路径形式嵌入 SVG，确保跨平台一致显示

### 7.7.5 图片排版

- 图片**有序排列**，按引用顺序编号
- 图片**不得重叠**：每张图有独立排版空间
- 图片**不得挤压**：留足间距，文字与图不挤在一起
- 图注紧跟图片下方，与正文引用保持同步

---

## 7.8 交付回复：权利要求偏向点（建议交互，必做）
